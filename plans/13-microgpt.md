...
*Under Plan 14, training rewards are minted as Pedersen commitments rather than plaintext ledger entries. The reward amount remains public (tied to the verifiable training run in `training_runs`), but the recipient's accumulated balance is private. Inference costs are deducted via private transfers — the agent's Fuchi-equivalent generates a proof that the agent can afford the inference, without revealing total holdings. The `cost_koi` field in `inference_log` records the public cost (needed for market pricing); the actual payment is a nullified commitment.*
...
CREATE UNLOGGED TABLE kerai.inference_log (
    ...
    cost_koi      BIGINT DEFAULT 0,           -- nKoi (public cost for pricing; actual payment is a private commitment under Plan 14)
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
```
